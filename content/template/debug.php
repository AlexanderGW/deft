...
<main>
	<div class="debug">
		...
		___( 'Snappy run time: %1$4.4f seconds. Memory usage: %2$s', $window, Helper::getShnoFromBytes( memory_get_usage() ) );

		$instances = $plugins = $captures = $hooks = $filters = $others = array();
		foreach( Snappy::getLog() as $stack => $events ) {
			$scope = explode( '/', $stack );

			if( strpos( $stack, 'instance/' ) === 0 ) {
